...
if($_SERVER['REQUEST_URI'] == "/rss/global/recently_added.rss" || $_SERVER['REQUEST_URI'] == "/rss/") {
	...
	$videos = $conn->query("SELECT * FROM videos LEFT JOIN users ON users.uid = videos.uid WHERE videos.converted = 1 AND videos.privacy = 1 ORDER BY videos.uploaded DESC LIMIT 15");
	...
	if($user->rowCount() > 0) {
		...
		$videos = $conn->prepare("SELECT * FROM videos LEFT JOIN users ON users.uid = videos.uid WHERE videos.uid = ? AND videos.converted = 1 AND videos.privacy = 1 ORDER BY videos.uploaded DESC LIMIT 15");
	...
	$videos = $conn->prepare("SELECT * FROM videos LEFT JOIN users ON users.uid = videos.uid WHERE videos.tags REGEXP ? AND videos.converted = 1 AND videos.privacy = 1 ORDER BY videos.uploaded DESC"); // Regex!
