...
// workaround to the font not appearing
$font = dirname(__FILE__) . '/img/ProFontWindows.ttf';
...
$textSize = 32;
$outlineSize = 1;
$textBox = imagettfbbox($textSize, 0, $font, $captchaText);
...
for ($x = $textX - $outlineSize; $x <= $textX + $outlineSize; $x++) {
    for ($y = $textY - $outlineSize; $y <= $textY + $outlineSize; $y++) {
        imagettftext($captchaImage, $textSize, 0, $x, $y, $outlineColor, $font, $captchaText);
    }
}
...
for ($y = 0; $y < $textHeight; $y++) {
    ...
    imagettftext($captchaImage, $textSize, 0, $textX, $textY, $gradientTextColor, $font, $captchaText);
}
