...
if($profile_latest_video->rowCount() == 0) {
	...
	"SELECT * FROM videos
	LEFT JOIN users ON users.uid = videos.uid
	WHERE videos.uid = ? AND videos.converted = 1 AND videos.privacy = 2
...
$vidocount = $vidocount->rowCount();
	...
	"SELECT * FROM videos
	LEFT JOIN users ON users.uid = videos.uid
	WHERE videos.uid = ? AND videos.converted = 1 AND privacy = 2
...
<table align="center" cellpadding="0" cellspacing="0" border="0">
	<tr>
		...
		<td style="padding: 0px 5px 0px 5px;">|</td>
		<td><strong>Private Videos</strong> (<?php echo $profile['priv_videos']; ?>)</td>
		<td style="padding: 0px 5px 0px 5px;">|</td>
		...
		<table width="100%" align="center" cellpadding="0" cellspacing="0" border="0" bgcolor="#CCCCCC">
			...
			<tr>
				...
				<table width="100%" cellpadding="0" cellspacing="0" border="0">
					<tbody><tr valign="top">
						<td><div class="moduleTitle">Private Videos // <?php echo htmlspecialchars($profile['username']) ?></div></td>
						<td align="right">
                ...
                <div class="moduleEntry"> 
					<table width="100%" cellpadding="0" cellspacing="0" border="0">
						<tbody><tr valign="top">
							...
							<td width="100%"><div class="moduleEntryTitle" style="word-break: break-all;"><a href="watch.php?v=<?php echo $video['vid']; ?>&search=<? echo htmlspecialchars($profile['username']); ?>"><?php echo htmlspecialchars($video['title']); ?></a></div>
...
<div style="font-size: 13px; font-weight: bold; color: #444; text-align: right; padding: 5px 0px 5px 0px;">Result Page:
    ...
    <a href="results.php?user=<? echo htmlspecialchars($profile['username']); ?>&page=<?php echo $_GET['page'] - 1; ?>"> < Previous</a>
    <?php } ?>
    ...
    for ($i = $startPage; $i <= $endPage; $i++) {
        if ($i == $page) {
            ...
            echo '<span style="background-color: #CCC; padding: 1px 4px 1px 4px; border: 1px solid #999; margin-right: 5px;"><a href="profile_videos_private.php?user=' . htmlspecialchars($profile['username']) . '&page=' . $i . '">' . $i . '</a></span>';
        }
    }
    ?>
    <!-- Add "Next" link if there are more pages -->
    <?php if ($endPage < $totalPages) { ?>
            <a href="profile_videos_private.php?user=<? echo htmlspecialchars($profile['username']); ?>&page=<?php echo $_GET['page'] + 1; ?>">Next ></a>
    <?php } ?>
</div>
<?php } ?>
				<!-- end paging -->
