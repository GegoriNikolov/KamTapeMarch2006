...
<table align="center" cellpadding="0" cellspacing="0" border="0">
	<tr>
		...
		<td><strong>Public Videos</strong> (<?php echo $profile['videos']; ?>)</td>
		<td style="padding: 0px 5px 0px 5px;">|</td>
		<td><a href="profile_videos_private.php?user=<?php echo htmlspecialchars($profile['username']) ?>">Private Videos</a> (<?php echo $profile['priv_videos']; ?>)</td>
		<td style="padding: 0px 5px 0px 5px;">|</td>
		...
		<td style="padding: 0px 5px 0px 5px;">|</td>
		<td><a href="profile_friends.php?user=<?php echo htmlspecialchars($profile['username']) ?>">Friends</a></td>
	</tr>
</table><p>
		...
		<table width="100%" align="center" cellpadding="0" cellspacing="0" border="0" bgcolor="#CCCCCC">
			...
			<tr>
				...
				<table width="100%" cellpadding="0" cellspacing="0" border="0">
					<tbody><tr valign="top">
						<td><div class="moduleTitle">Public Videos // <?php echo htmlspecialchars($profile['username']) ?></div></td>
						<td align="right">
						<? if($vidocount > 0) { ?><div style="font-weight: color: #444; margin-right: 5px;"><b>Videos <?php if ($offs > 0) { echo htmlspecialchars(trim($offs)); } else { echo "1"; } ?>-<? if($vidocount > $ppv) { $nextynexty = $offs + $ppv; } else {$nextynexty = $vidocount; } echo htmlspecialchars($nextynexty); ?> of <?php                               echo $vidocount; ?></b></div><? } ?>
						</td>
					</tr>
				</tbody></table>
				</div>
...
?></div>
							...
							<div class="moduleEntryDetails">Runtime: <?php echo gmdate("i:s", $video['time']); ?> | Views: <?php echo $video['views']; ?> | Comments: <?php echo $video['comments']; ?></div>
...
<div style="font-size: 13px; font-weight: bold; color: #444; text-align: right; padding: 5px 0px 5px 0px;">Result Page:
    ...
    <a href="profile_videos.php?user=<? echo htmlspecialchars($profile['username']); ?>&page=<?php echo $_GET['page'] - 1; ?>"> < Previous</a>
    <?php } ?>
    ...
    for ($i = $startPage; $i <= $endPage; $i++) {
        if ($i == $page) {
            ...
            echo '<span style="background-color: #CCC; padding: 1px 4px 1px 4px; border: 1px solid #999; margin-right: 5px;"><a href="profile_videos.php?user=' . htmlspecialchars($profile['username']) . '&page=' . $i . '">' . $i . '</a></span>';
        }
    }
    ?>
    <!-- Add "Next" link if there are more pages -->
    <?php if ($endPage < $totalPages) { ?>
            <a href="profile_videos.php?user=<? echo htmlspecialchars($profile['username']); ?>&page=<?php echo $_GET['page'] + 1; ?>">Next ></a>
    <?php } ?>
</div>
<?php } ?>
				<!-- end paging -->
		...
		<a href="http://www.kamtape.com/rss/user/<? echo htmlspecialchars($profile['username']); ?>/my_videos.rss"><img src="img/rss.gif" width="36" height="14" border="0" style="vertical-align: text-top;"></a> 
		<span style="font-size: 11px; margin-right: 3px;"><a href="http://www.kamtape.com/rss/user/<? echo htmlspecialchars($profile['username']); ?>/my_videos.rss">Feed For User // <? echo htmlspecialchars($profile['username']); ?></a></span>
		...
		<div style="padding: 0px 0px 5px 0px; color: #999;">&#187; <a href="profile_videos.php?user=<?php echo htmlspecialchars($tag); ?>"><?php echo htmlspecialchars($tag); ?></a></div>
		<?php } ?>
